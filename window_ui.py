...
from communication import check_connect, create_connection, get_hardware_basic_info, init_sensor_touch
import draw
...
class MainUI:
    def __init__(self, root):
        ...
        for i, name in enumerate(buttons.keys()):
            self.cmd_btn.append(Button(self.cmd_list, text=name, state=DISABLED, command=lambda name=name: self.execute_command(name, buttons[name])))
            if canvas_size == 1080:
    ...
    def click_start_connect(self):
        ...
        if stat == (1,):
            ...
            init_sensor_touch()
            threading.Thread(target=self.daemon_draw_text, daemon=True).start()
            threading.Thread(target=self.daemon_draw_touch, daemon=True).start()
            return
